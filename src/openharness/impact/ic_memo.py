...
def render_ic_memo_html(
    ...
    *,
    ...
    if greenwashing_score is not None:
        ...
        if verdict_card is not None:
            findings = "".join(f"<li>{escape(str(item))}</li>" for item in verdict_card.top_3_findings)
            strengths = "".join(f"<li>{escape(str(item))}</li>" for item in verdict_card.top_3_strengths)
            steps = "".join(f"<li>{escape(str(item))}</li>" for item in verdict_card.next_steps)
            verdict_kind = {"pass": "pass", "caution": "warn", "fail": "fail"}.get(
                verdict_card.verdict, "neutral",
            )
            extras_html.append(
    ...
    if proof_appendix is not None:
        ...
        if getattr(proof_appendix, "assumptions", None):
            ...
            proof_parts.append(f'<div class="callout warn"><b>Assumptions</b><ul>{items}</ul></div>')
        if getattr(proof_appendix, "evidence_warnings", None):
            items = "".join(f"<li>{escape(str(item))}</li>" for item in proof_appendix.evidence_warnings)
            proof_parts.append(f'<div class="callout"><b>Evidence warnings</b><ul>{items}</ul></div>')
