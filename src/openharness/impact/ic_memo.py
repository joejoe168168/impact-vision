...
from openharness.impact.deal_gate import DealScorecard, TargetCondition
...
def render_ic_memo_markdown(
    ...
    *,
    ...
    if thesis and not thesis.is_default:
        out.append(
            f"Fund thesis: *{thesis.name}* ({thesis.strategy}, vintage {thesis.vintage_year})"
        )
        if fd:
            w5d = weighted_5d_overall(fd, thesis)
            out.append(
                f"- Fund-weighted 5D overall: **{w5d:.2f}/5** (unweighted: {_fmt_score(fd.overall_score)})"
            )
        if sdgs:
        ...
        out.append(
            "*No fund thesis loaded — using equal-weight defaults. Set `data/fund_thesis.yaml` for fund-specific weighting.*"
        )
    out.append("")
    ...
    if fd:
        ...
        for dim_name, ds in (
            ("What", fd.what),
            ("Who", fd.who),
            ("How Much", fd.how_much),
            ("Contribution", fd.contribution),
            ("Risk", fd.risk),
        ):
            out.append(f"| {dim_name} | {_fmt_score(ds.score)} | {ds.provenance} | {ds.notes} |")
        out.append(
            f"| **Overall** | **{_fmt_score(fd.overall_score)}** | {fd.overall_provenance} | Grade: {fd.overall_grade} |"
        )
    ...
    if greenwashing_score is not None:
        out.append(
            f"- Composite risk: **{greenwashing_score:.1f}/100**"
            + (f" — {greenwashing_classification}" if greenwashing_classification else "")
        )
        if verdict_card is not None:
            out.append(f"- Verdict: **{str(verdict_card.verdict).upper()}**")
            if getattr(verdict_card, "top_3_findings", None):
                out.append("")
                out.append("**Top concerns:**")
                for item in verdict_card.top_3_findings:
                    out.append(f"- {item}")
            if getattr(verdict_card, "next_steps", None):
                out.append("")
                out.append("**Recommended next steps:**")
                for item in verdict_card.next_steps:
                    out.append(f"- {item}")
    else:
    ...
    out.append("## 7. Impact targets as investment conditions")
    if target_conditions:
        out.append("| Metric | Baseline | Target | By period | Condition | Linked incentive |")
        out.append("|---|---:|---:|---|---|---|")
        for target in target_conditions:
            baseline = "n/a" if target.baseline is None else str(target.baseline)
            out.append(
                f"| {target.metric_id} | {baseline} | {target.target_value} | "
                f"{target.by_period} | {target.condition_kind} | "
                f"{target.linked_incentive or ''} |"
            )
    else:
        out.append("*No binding impact targets were set as investment conditions.*")
    out.append("")
    ...
    if proof_appendix is not None:
        out.append("## Appendix A. Proof of Rigor")
        out.append("")
        if getattr(proof_appendix, "reported_metrics", None):
            out.append("### Reported Metrics")
            out.append("| Metric | Value | Unit | Source | Verification | Evidence refs |")
            out.append("|---|---:|---|---|---|---|")
            for metric in proof_appendix.reported_metrics:
                refs = ", ".join(getattr(metric, "evidence_refs", []) or [])
                out.append(
                    f"| {metric.metric_id} | {metric.value} | {metric.unit or ''} | "
                    f"{metric.source or ''} | {metric.verification_status} | {refs} |"
                )
            out.append("")
        if getattr(proof_appendix, "unmeasured_claims", None):
            out.append("### Unmeasured Claims")
            out.append("| Claim | Gap | Why it matters |")
            out.append("|---|---|---|")
            for claim in proof_appendix.unmeasured_claims:
                out.append(f"| {claim.claim} | {claim.metric_gap} | {claim.why_it_matters} |")
            out.append("")
        if getattr(proof_appendix, "assumptions", None):
            out.append("### Assumptions")
            for item in proof_appendix.assumptions:
                out.append(f"- {item}")
            out.append("")
        if getattr(proof_appendix, "evidence_warnings", None):
            out.append("### Evidence Warnings")
            for item in proof_appendix.evidence_warnings:
                out.append(f"- {item}")
            out.append("")
...
def render_ic_memo_pptx(
    ...
    if assessment.five_dimensions:
        fd = assessment.five_dimensions
        body = (
            f"Overall: {fd.overall_score:.1f}/5 ({fd.overall_grade})\n"
...
def render_ic_memo_html(
    ...
    *,
        ...
        kpis.append(
            {
                ...
                "kind": "pass"
                if fd.overall_score >= 3.5
                else "warn"
                if fd.overall_score >= 2.5
                else "neutral",
            }
        )
    if top:
        ...
        kpis.append(
            {
                ...
                "kind": "pass"
                if dd_coverage_pct >= 70
                else "warn"
                if dd_coverage_pct >= 40
                else "fail",
            }
        )
    ...
    # --- Section: Thesis fit ---------------------------------------
    thesis_html: list[str] = [
        '<section class="card" id="thesis-fit"><h2 class="section-title">Impact thesis fit</h2>'
    ]
    if thesis and not thesis.is_default:
        thesis_html.append(
    ...
    # --- Section: 5-Dimension ---------------------------------------
    five_d_html: list[str] = [
        '<section class="card" id="five-d"><h2 class="section-title">5-Dimension impact score</h2>'
    ]
    if fd:
        rows: list[str] = []
        for dim_name, ds in (
    ...
    extras_html: list[str] = [
        '<section class="card" id="dd-gw"><h2 class="section-title">DD & greenwashing</h2>'
    ]
    if dd_coverage_pct is not None:
    ...
    if greenwashing_score is not None:
        ...
        extras_html.append(
            ...
            + (
                ...
                if greenwashing_classification
                else ""
            )
            + "</p>",
        )
        if verdict_card is not None:
            findings = "".join(
                f"<li>{escape(str(item))}</li>" for item in verdict_card.top_3_findings
            )
            strengths = "".join(
                f"<li>{escape(str(item))}</li>" for item in verdict_card.top_3_strengths
            )
            steps = "".join(f"<li>{escape(str(item))}</li>" for item in verdict_card.next_steps)
            verdict_kind = {"pass": "pass", "caution": "warn", "fail": "fail"}.get(
                verdict_card.verdict,
                "neutral",
            )
            extras_html.append(
                f'<div class="callout {verdict_kind}"><b>Impact credibility verdict: '
                f"{escape(str(verdict_card.verdict).upper())}</b>"
                f"<p>{escape(str(verdict_card.summary))}</p>"
                f'<div class="cards-row"><div><h4>Concerns</h4><ul>{findings}</ul></div>'
                f"<div><h4>Strengths</h4><ul>{strengths}</ul></div>"
                f"<div><h4>Next steps</h4><ul>{steps}</ul></div></div></div>",
            )
    else:
    ...
    if proof_appendix is not None:
        ...
        if getattr(proof_appendix, "reported_metrics", None):
            rows = "".join(
                f"<tr><td>{escape(metric.metric_id)}</td>"
                f"<td>{escape(str(metric.value))}</td>"
                f"<td>{escape(str(metric.unit or ''))}</td>"
                f"<td>{escape(str(metric.source or ''))}</td>"
                f"<td>{escape(str(metric.verification_status))}</td></tr>"
                for metric in proof_appendix.reported_metrics
            )
            proof_parts.append(
                '<h3>Reported metrics</h3><table class="data"><thead><tr>'
                "<th>Metric</th><th>Value</th><th>Unit</th><th>Source</th>"
                f"<th>Verification</th></tr></thead><tbody>{rows}</tbody></table>",
            )
        if getattr(proof_appendix, "unmeasured_claims", None):
            rows = "".join(
                f"<tr><td>{escape(claim.claim)}</td><td>{escape(claim.metric_gap)}</td>"
                f"<td>{escape(claim.why_it_matters)}</td></tr>"
                for claim in proof_appendix.unmeasured_claims
            )
            proof_parts.append(
                '<h3>Unmeasured claims</h3><table class="data"><thead><tr>'
                f"<th>Claim</th><th>Gap</th><th>Why it matters</th></tr></thead><tbody>{rows}</tbody></table>",
            )
        if getattr(proof_appendix, "assumptions", None):
            items = "".join(f"<li>{escape(str(item))}</li>" for item in proof_appendix.assumptions)
            proof_parts.append(
                f'<div class="callout warn"><b>Assumptions</b><ul>{items}</ul></div>'
            )
        if getattr(proof_appendix, "evidence_warnings", None):
            items = "".join(
                f"<li>{escape(str(item))}</li>" for item in proof_appendix.evidence_warnings
            )
            proof_parts.append(
                f'<div class="callout"><b>Evidence warnings</b><ul>{items}</ul></div>'
            )
        proof_parts.append("</section>")
        proof_html = "".join(proof_parts)
    ...
    gate_html: list[str] = [
        '<section class="card" id="gate"><h2 class="section-title">IC gate detail</h2>',
    ...
    target_rows = "".join(
        "<tr>"
        f"<td>{escape(target.metric_id)}</td>"
        f"<td>{escape(str(target.baseline if target.baseline is not None else 'n/a'))}</td>"
        f"<td>{target.target_value}</td>"
        f"<td>{escape(target.by_period)}</td>"
        f"<td>{escape(target.condition_kind)}</td>"
        f"<td>{escape(target.linked_incentive or '')}</td>"
        "</tr>"
        for target in (target_conditions or [])
    )
    targets_html = [
        '<section class="card" id="target-conditions">'
        '<h2 class="section-title">Impact targets as investment conditions</h2>'
    ]
    if target_rows:
        targets_html.append(
            '<table class="data"><thead><tr><th>Metric</th><th>Baseline</th>'
            "<th>Target</th><th>By period</th><th>Condition</th>"
            f"<th>Linked incentive</th></tr></thead><tbody>{target_rows}</tbody></table>"
        )
    else:
        targets_html.append(
            '<div class="callout warn">No binding impact targets were set as '
            "investment conditions.</div>"
        )
    targets_html.append("</section>")
    ...
    toc_items = [
        ...
        ("target-conditions", "6. Impact target conditions"),
        ("rec", "7. Recommendation"),
    ]
    if proof_appendix is not None:
        toc_items.append(("proof", "7. Proof of rigor"))
